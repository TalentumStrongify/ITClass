...
public class Registro extends MVCPortlet {
	...
	private static final String procesarRegistro= "procesarRegistro";
	...
	public void procesarRegistro(ActionRequest actionRequest, ActionResponse actionResponse)
		...
		try {
			...
			// Setear la informacion del usuario 
			...
			// Agregar la informacion del usuario a la BBDD
	...
	public void recogerDatosEncuesta(ActionRequest actionRequest, ActionResponse actionResponse)
		...
		//recuperamos idColegio e idRol de la tabla entidad Usuario en BBDD
		int idce = usuario.getIdce();
		int idrol = usuario.getIdrol();

		// inicializamos un array que recogera iterativamente los datos de la encuesta
		...
		//el id encuesta, es el numero de preguntas que hay registradas en la BBDD (clave única)
		//cada vez que se complete un cuestionario aumenta.
		int p=EvaluacionesCELocalServiceUtil.getEvaluacionesCEsCount();
		
		EvaluacionesCEImpl evaluacion = new EvaluacionesCEImpl();
		//Recuperamos en el siguiente bucle la respuesta a cada pregunta de la encuesta 
		for (int i = 0; i < PreguntasEncuestaLocalServiceUtil.getPreguntasEncuestasCount(); i++) {
			...
			try { 
				//Hacemos set y añadimos los datos de cada pregunta en la BBDD
				...
				//vuelve a la encuesta en caso de no rellenar la encuesta correctamente
				actionResponse.setRenderParameter(VISTA_DESTINO, "view");
			}
...
//constructor del User de Liferay
